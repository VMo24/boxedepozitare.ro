...
function bindLoadArticles() {
    ...
    axios.get(`/getArticlePage/${currentPage}`)
    .then(function (response) {
      // handle success
      if(response.data.articles.length>0){
        response.data.articles.forEach(element => {
            let continut = JSON.parse(element.continut)
            setTimeout(()=>{articlesContainer.innerHTML+=parseArticle(element.imagine, element.id2, continut.div[0].h2 ,continut.div[0].p , continut.div[1].p);},600)
        });
        currentPage++;
